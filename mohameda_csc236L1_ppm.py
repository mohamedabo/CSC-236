...
'''
    ...
    def PPM_makeoutputfiles(self):
        ...
        intempfile.close()
        outtmpfile.close()
        # create the ascii file first, and then reload and output to a binary file.
        ...
        for rowlist in self.pixellist:
            for pixel in rowlist:
                for color in pixel:
                    ...
                    output.append(color) # create a list of numbers, to be converted to binary later

        strout=header+strout+'\n'
        tmpfile = open(self.outbinfile, "wb")
        tmpfile.write(bytes(header,'utf-8'))    # must convert into bytes for the binary file for Python 3.X
        tmpfile.write(bytes(output))            # must convert into bytes for the binary file for Python 3.X
        tmpfile.close() #close tmpfile when done
    ...
    #         col = 0
    #         for pixel in rowlist:
    #             variable = (newpixellist[row][col][0] + newpixellist[row][col][1] + newpixellist[row][col][2])//3
    #             newpixellist[row][col][1] = variable # update green
    #             newpixellist[row][col][2] = variable # update blue
    #             newpixellist[row][col][0] = variable # update blue
    #             col += 1
    #         row += 1
    ...
    #     newpixellist = self.pixellist
    #     new_pic = copy.deepcopy(newpixellist) # creating a deep copy
    #     rotated_pic = []
    #     for column in range (self.width):
    #         rotated_pic.append([])
    #         for pixel in range (self.height-1,-1,-1):
    #             rotated_pic[column].append(new_pic[pixel][column])
    #     print(self.outasciifile + "Output file has been rotated clockwise")
    def stringtoaschii(self):
        """Converts the string to the aschii values"""
        m = input("Enter text file: ")
        msg = open(m,"r")
        filename= msg.readline()
        self.aschii =[]
        for i in filename:
            for index in i:
                self.aschii.append(ord(index)) # getting the aschii values
        return self.aschii

    def PPM_encode_image(self):
        """Hides the messagge inside the image"""
        newpixellist = self.pixellist
        index = 0
        row = 0
        self.height = len(newpixellist)
        self.width = len(newpixellist[2])
        numbers = self.aschii # the list of the aschii values
        numbers.append(0) # appending the condition to stop the iteration through the text
        for rowlist in newpixellist:
            col = 0
            if index<= len(numbers)-1:
                for pixel in rowlist:
                    if index<= len(numbers)-1:
                        newpixellist[row][col][2]= int(numbers[index]) # converting the aschii values to integers
                        index+=1
                        col+=1
                    else:
                        break
                ...
                break
            row+=1
        return self.PPM_updatefrompixellist(newpixellist)

    def PPM_decoded_image(self):
        """Decodes the hidden messagge inside the image"""
        newpixellist = self.pixellist
        self.width = len(newpixellist[0])
        self.height= len(newpixellist)
        decoded_messagge = []
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                messagge = newpixellist[row][col][2]
                col+=1
                if messagge==0: # if the message reached to element zero
                    break # stop iterating through the text
                hidden_letter = chr(int(messagge)) # converting the aschii values to charachters
                decoded_messagge.append(hidden_letter)
            row+=1
            if messagge==0:
                break

        print(decoded_messagge)
        return decoded_messagge



         # This call will update all member attributes appropriately.
